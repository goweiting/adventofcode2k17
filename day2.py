...
def part_one():
  ...
  total = 0
  for row in rows:
    row = map(int, row.split())  # convert to int
    largest = max(row)
    smallest = min(row)
  ...
  for row in rows:
    ...
    for i, num in enumerate(row):
      if found:
        ...
        # all the remaining integers to compare with
        nums_to_check = row[i + 1:]
        for num_to_check in nums_to_check:
          print(num, num_to_check)
          if num > num_to_check and (num % num_to_check) == 0:
            print(num, num_to_check)
            total += num / num_to_check
            found = True
            break
          else:
            if (num_to_check % num) == 0:
              total += num_to_check / num
              found = True
              break
  return total
